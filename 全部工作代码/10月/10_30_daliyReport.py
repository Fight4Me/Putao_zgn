...
def send_email(today_task_number,total_finished_task_number,proportion,today_active_number,total_tagging_people,str5,str6,today_verified_task_number,verified_proportion,str9,html_path,html2_path,str12,str13):
    #链接邮箱服务器
    ...
    html += '<a href =\'http://193.112.75.135:8000/tagging/report/\'>点我看近十日标注量变化曲线</a><br>'
    html += '<h3>审核情况</h3>'
    html += '当日审核完成数量：'+today_verified_task_number+'<br>'
    html += '审核进度：'+verified_proportion+'<br>'
    html += '<a href =\'http://193.112.75.135:8000/tagging/report2/\'>点我看近十日审核量变化曲线</a><br>'
    html += '<h3>今日工资结算情况</h3>'
    if str13:
...
def echarts_plot(html_path,html2_path,col_1,row_1,col_2,row_2):
    ...
    line.add("日期", row_2, col_2 , mark_point=[ "max", "min"], mark_line=["average"],is_datazoom_show=False,is_symbol_show=True)
...
sql11 = 'SELECT count(*), event_date from Tagging_event where event_type = 5  GROUP BY event_date ORDER BY event_date ASC LIMIT 10'
...
sql = [sql1,sql2_1,sql2_2,sql3,sql4,sql5,sql6,sql7,sql8,sql9,sql10,sql11,sql12,sql13]
...
html2_path = r'/search/heyu/web/TaggingSys/sb-admin/snapshot2.html'
...
#        print(str13)
        ...
        send_email(today_task_number,total_finished_task_number,proportion,today_active_number,total_tagging_people,str5,str6,today_verified_task_number,verified_proportion,str9,html_path,html2_path,str12,str13)
